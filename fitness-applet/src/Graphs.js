    ...
    return (
      <div style={styles.outer} >
        <div style={styles.inner}>
          <ResponsiveContainer>
            {this.props.barGraph ? <BarGraph data={data} /> : this.props.otherGraph ? <LineGraph data={data} /> : <p>Please define a graph type</p>}
          </ResponsiveContainer>
        </div>
      </div>
    )
...
class CustomizedDot extends Component {
  render() {
    const { cx, cy, stroke, payload, value, color } = this.props;
...
class LineGraph extends Component {
  render() {
    let graphOption = workoutName.map((option, i) => {
      return <Line key={`Option_${option}`} connectNulls={true} dot={<CustomizedDot color={colorChanger(i)} />} label={{ position: 'top', fontSize: 11 }} type="monotone" dataKey={option} stroke={colorChanger(i)} />
    });
    return (
      <ResponsiveContainer>
